...
Class AdminStudentsController extends \BaseController {
	...
	public function postStudentsExcelImport()
	{
		...
		//Excel::load('imports/'.$filename, function($reader) {
 		...
 		//	$results = $reader->select(array('card_number','rollno','rol_no','contact_number','first_name','last_name','dob','college_id','streamorcourse','validity_for_how_many_years','cluborgrouporsociety','residentordayscholar','date_of_issue','expiry_date','section','father_name','batch_year','program_duration','email_id','gender'))->get();
		...
		// echo"<pre>";print_r($results);exit();
			...
			foreach($results as $result)
			{
				...
				$dob 		 		 = $result['dob']; //echo"<pre>";print_r($dob->format('d-m-Y') );exit();

				$date_of_issue 		 = $result['date_of_issue']; //echo"<pre>";print_r($dob->format('d-m-Y') );exit();

				$expiry_date 		 = $result['expiry_date']; //echo"<pre>";print_r($dob->format('d-m-Y') );exit();
	...
	//	});
		...
		// echo"<pre>";print_r($success);print_r($input);exit();
	...
	public function getStudentUsersDataSampleExcel()
	{
		Excel::create('StudentUsersDataSample', function($excel) {
			...
			$excel->sheet('StudentUsersDataSample', function($sheet) {
				...
				$sheet->fromArray(array(array('card_number', 'rollno', 'rol_no', 'contact_number', 'first_name', 'last_name', 'dob', 'college_id', 'streamorcourse', 'validity_for_how_many_years', 'cluborgrouporsociety', 'residentordayscholar', 'date_of_issue', 'expiry_date', 'section', 'father_name', 'batch_year', 'program_duration', 'email_id', 'gender'),
				array('1234123412341234','1234123','123123','1231231231','john','legend','11-03-1993','171','B. Architecture','1','','DayScholar','01-01-2015','31-12-2015','X78492','M.B.Pulami','2010','5','aman@gmail.com','M'),
				),null,'A1',false,false);
					...
					foreach($studentsone as $key){ 
						...
						//Mail will goes here
							...
							if(count($studentTemplate)>0 && !empty($studentTemplate[0]->content)){
								...
								$imageName = $this->imageDynamic($bimage, $card_number, $name, $validate, $rollnm);
								$array = array();
							...
							$imageName = imageDynamic('default_idoag_card_design.png', $card_number, $name, $validity, $rollnumber);
								$array = array();
								array_push($array, $key->email_id);
								array_push($array, $key->first_name);
								array_push($array, $imageName);
						...
						//Mail will goes here
							...
							if(count($studentTemplate)>0 && !empty($studentTemplate[0]->content)){
							 ...
							 Mailgun::send('emails.emailTemplate', $data, function($message) use($array)
								...
								$imageName = $this->imageDynamic('default.png', $card_number, $name, $validate, $rollnumber);
								$array = array();
								array_push($array, $key->email_id);
								array_push($array, $key->first_name);
								array_push($array, $imageName);
	...
	public function imageDynamic($backImage, $card_number, $name, $validity, $rollno){
		...
		imagestring( $my_img, 40, 30, 190, $cardNumber, $text_colour );
		imagestring( $my_img, 40, 30, 220, $name, $text_colour );
		imagestring( $my_img, 40, 30, 245, "VALID UPTO : ".$validity, $text_colour );
		if(!empty($rollno)){imagestring( $my_img, 40, 30, 260, "ROLL NO : ".$rollno, $text_colour );}
		$imageName = time().'.png';
