...
class C5AB_twitter_slider extends C5_Widget {
	...
	function shortcode($atts,$content) {
		    ...
		    if (!empty($c5ab_twitter_tweets)) {
		        $data = $data . '<div class="c5ab_twitter_slider"><ul class="c5ab_twitter_slides">';
		        
		        foreach ($c5ab_twitter_tweets as $tweet) {
		            $data = $data . '<li><p><span class="fa fa-twitter"> </span> <span class="text">' . $this->convert_links($tweet['text']) . '</span><br /><a class="twitter_time" target="_blank" href="http://twitter.com/' . $atts['username'] . '/statuses/' . $tweet['status_id'] . '">' . $this->relative_time($tweet['created_at']) . '</a></p></li>';
		        }
